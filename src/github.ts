...
export async function getIssue(
  ...
  if (!res.ok) {
    throw new Error(
      `GitHub get issue failed: ${res.status} ${res.statusText}`,
    );
  }
...
export async function updateIssueState(
  ...
  if (!res.ok) {
    throw new Error(
      `GitHub update issue failed: ${res.status} ${res.statusText}`,
    );
  }
  return res.json() as Promise<GitHubIssue>;
}
...
export async function listComments(
  ...
  if (!res.ok) {
    throw new Error(
      `GitHub list comments failed: ${res.status} ${res.statusText}`,
    );
  }
...
export async function createComment(
  ...
  if (!res.ok) {
    throw new Error(
      `GitHub create comment failed: ${res.status} ${res.statusText}`,
    );
  }
  return res.json() as Promise<GitHubComment>;
}
...
export function parseGitHubIssueRef(
  ...
  // #number format (needs default repo)
